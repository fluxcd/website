...
An ignore rule is not just about sharing ownership after the fact: because the value committed to
Git is applied when the object is first created, it also lets Flux **seed an initial value that
another controller then takes over**. Flux sets the field on the first apply, and once another
controller writes to it, Flux steps back and preserves that value on every subsequent
reconciliation.
...
This also improves on the long-standing guidance of omitting `replicas` from Git so Flux would not
fight the autoscaler: an omitted field defaults to `1`, so a fresh rollout starts at a single
replica until the HPA scales it up. With an ignore rule you can instead commit a sensible starting
value, for example `replicas: 10`. Flux seeds that value when the Deployment is first created, and
the HPA takes over from there.
