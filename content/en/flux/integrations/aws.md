...
- The image-reflector-controller integrates the `ImageRepository` and `ImagePolicy` APIs with
  ECR for scanning tags and digests of OCI artifacts and reflecting them into the cluster.
...
ECR. The `OCIRepository` API can be used to pull OCI
artifacts from ECR into the cluster, while the `ImageRepository` and `ImagePolicy` APIs
...
require secret-based authentication, which is less secure. Both secret-less
and secret-based authentication are supported at the object and controller
levels, except for the `Bucket` and `Provider` APIs that will only support
secret-less authentication at the object level in the next minor release
(Flux v2.7).
...
authentication through OIDC Federation at the object level. Support for these
APIs will be introduced in Flux v2.7.
...
Only for the ECR integrations we support authentication
...
ECR in order to start pods. By supporting this authentication method we allow
...
> but we only have continuous integration tests for the ECR integration in
