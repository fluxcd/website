...
- The image-reflector-controller integrates the `ImageRepository` and `ImagePolicy` APIs with
  ACR for scanning tags and digests of OCI artifacts and reflecting them into the cluster.
- The source-controller integrates the `GitRepository` API with
  [Azure DevOps (ADO)](https://learn.microsoft.com/en-us/azure/devops/user-guide/services?view=azure-devops)
  for pulling manifests from Git repositories and packaging them as artifacts inside the cluster.
- The image-automation-controller integrates the `ImageUpdateAutomation` API with
  ADO for automating image updates in Git repositories.
- The notification-controller integrates the `Provider` API with ADO for updating
  commit statuses in Git repositories upon events related to Flux resources.
- The source-controller integrates the `Bucket` API with
...
The next sections briefly describe [Microsoft Entra ID](https://learn.microsoft.com/en-us/entra/fundamentals/whatis),
formerly known as *Azure Active Directory (AAD)*, Azure's identity and access management system,
and how it can be used to grant Flux access to resources offered by the services above. Bear in
mind that Microsoft Entra ID has more features than the ones described here. We describe only the
features that are relevant for the Flux integrations.

> **Note**: Flux also integrates with Microsoft Teams, but Microsoft Teams is not an Azure
> service and hence does not support Microsoft Entra ID in the forms that Flux can integrate
> with. For more information about the Microsoft Teams integration with notification-controller,
> see the `Provider` API docs.
...
GCP has a large number of predefined roles, but users can also create
...
granting IAM roles you can use one of the following custom resources:
...
To use Terraform/OpenTofu for granting IAM roles you can use one of the following resources:
...
To use the `gcloud` CLI for granting IAM roles you can use one of the following commands:
...
In this section we describe the recommended IAM roles and permissions for
...
GAR. The `OCIRepository` API can be used to pull OCI artifacts
from GAR into the cluster, while the `ImageRepository` and `ImagePolicy` APIs
can be used to scan tags and digests of OCI artifact repositories and reflect them in the
cluster.

The recommended IAM role containing the required permissions for the `OCIRepository`,
...
The recommended IAM roles containing the required permissions for the `Bucket` API are:
...
The `Kustomization` Flux API is integrated with AZKV.
The `Kustomization` API is used to apply manifests in the cluster, and it can use AZKV to
decrypt SOPS-encrypted secrets before applying them.

The recommended IAM role containing the required permissions for the `Kustomization` API is:
...
The `Provider` Flux API is integrated with Google Cloud Pub/Sub. The `Provider` API can be used
to send notifications about Flux resources to Pub/Sub topics.

The recommended IAM role containing the required permissions for the `Provider` API is:
...
Accounts support secret-based authentication. Both secret-less and secret-based
authentication are supported both at the object and controller levels, except
for the `Bucket` and `Provider` APIs that will only support secret-less
authentication at the object level in the next minor release (Flux v2.7).
...
This process is known as *OIDC federation*.
...
identity type used by default. In this case, IAM roles must be granted using the
...
GCP Service Accounts support private keys that can be used to generate temporary
access tokens. Such keys are exported in the JSON format containing the private key
itself and other metadata.
...
described [here](#supported-clusters).
...
At the moment, the `Bucket` and `Provider` APIs **do not support** configuring
authentication through Workload Identity Federation at the object level. Support
for these APIs will be introduced in Flux v2.7.
...
Only for the GAR integrations we support authentication
...
GAR in order to start pods. By supporting this authentication method we allow
...
> but we only have continuous integration tests for the GAR integration in
