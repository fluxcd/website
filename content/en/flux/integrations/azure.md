...
2. Set the `.spec.provider` field to `azure` in the Flux resource.
   ...
   `Kustomization` and `HelmRelease` APIs the field is
   `.spec.kubeConfig.provider`.
3. Use the `.spec.serviceAccountName` field to specify the name of the
   ...
   `.spec.kubeConfig.serviceAccountName`.
...
set the `.spec.kubeConfig.provider` field to `azure` and leave the
`.spec.kubeConfig.serviceAccountName` field unset.
