...
- The image-reflector-controller integrates the `ImageRepository` and `ImagePolicy` APIs with
  GAR for scanning tags and digests of OCI artifacts and reflecting them into the cluster.
...
> **Note**: Flux also integrates with Google Chat, but Google Chat is not a GCP
> service and hence does not support GCP IAM. For more information about the Google
> Chat integration with notification-controller, see the `Provider` API docs.
